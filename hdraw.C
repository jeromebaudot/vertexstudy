...
#include <TCanvas.h>
    ...
    TFile f("./data/continuumuu-33k-tree.root");
    TTree *outt = (TTree *)f.Get("outt");

    TCanvas *c1 = new TCanvas("c1", "Histograms", 80, 80, 900, 900);
    c1->Print("./histo/continuumuu-33k-histos.pdf[");
    ...
    TH1F *h1_nvtxreal4 = new TH1F("h1_nvtxreal4", "Number of Vertices by event;Number of Vertices", 14, 0, 14); //at least 2 reconstructible + inter particles
    outt->Draw("event.e_nvtxreal4 + event.e_nvtxreal3>>h1_nvtxreal4", "id==0");

    h1_nvtx->SetMaximum(25000);
    h1_nvtx->SetLineColor(860);
    h1_nvtx->SetFillColorAlpha(860, 0.3);
    h1_nvtx->Draw();
    h1_nvtxreal2->SetMaximum(25000);
    ...
    h1_nvtxreal3->SetLineColor(49);
    h1_nvtxreal3->SetFillColorAlpha(46, 0.45);
    h1_nvtxreal3->Draw();
    h1_nvtxreal4->SetLineColor(kCyan + 2);
    h1_nvtxreal4->SetFillColorAlpha(kCyan + 2, 0.55);
    h1_nvtxreal4->Draw("Sames");

    gPad->Update();
    TPaveStats *st24 = (TPaveStats *)h1_nvtxreal4->FindObject("stats");
    st2->SetY1NDC(0.94);
    st2->SetY2NDC(0.78);
    st24->SetY1NDC(0.75);
    st24->SetY2NDC(0.6);
    ...
    leg24->AddEntry(h1_nvtxreal3, "#splitline{Vertices with at least}{2 reconstructible particles (lvl3)} ", "f");
    leg24->AddEntry(h1_nvtxreal4, "#splitline{Vertices with at least}{#splitline{2 reconstructible or }{intermediate particles (lvl4)}}", "f");
    leg24->Draw();
    c1->Update();
    c1->Print("./histo/continuumuu-33k-histos.pdf");
    ...
    TH1F *h1_e_nRInter = new TH1F("h1_e_nRInter", "Nb of charged particles/Event;Nb of charged particles", 30, 0, 30); //Charged
    h1_e_nRInter->SetLineColor(kCyan + 2);                                                                             //charged color
    ...
    TH1F *h1_e_RInter = new TH1F("h1_e_RInter", "Number of recontructible charged particles/Event;Nb of reconstructible charged particles", 28, 0, 28); //Reconstrutible
    outt->Draw("event.e_nRInter>>h1_e_RInter", "id==0");

    TH1F *h1_RIntertracks = new TH1F("h1_RIntertracks", "Number of recontructible charged particles/Event;Nb of reconstructible charged particles", 28, 0, 28); //Charged that belong to vertices level 3
    ...
    leg4Inter->AddEntry(h1_e_RInter, "#splitline{Intermediate}{charged particles}", "f");
    ...
    TH1F *h1_v_RInter = new TH1F("h1_v_RInter", "Number of reconstructible of intermedites particles/Vertex;Nb of particles", 20, 0, 20);
    ...
    TH1F *h1_pdg_realInter = new TH1F("h1_pdg_realInter", "Total Number of Vertices/PDG ID code;PDG ID code", 100000, 0, 100000); //PDG code of reconstructible vertices
    ...
    //**************************/ /
        ...
        TH1F *h1_rad_all = new TH1F("h1_rad_all", "Radial distance of vertices; distance [cm]", 200, 0, 10);
    outt->Draw("radialpos>>h1_rad_all");
    c1->SetLogy();

    TH1F *h1_rad_real = new TH1F("h1_rad_real", "Radial distance of vertices; distance [cm]", 200, 0, 10);
    outt->Draw("radialpos>>h1_rad_real", "vReal==3");
    c1->SetLogy();

    TH1F *h1_rad_pipe = new TH1F("h1_rad_pipe", "Radial distance of vertices; distance [cm]", 200, 0, 10);
    ...
    TH1F *h1_rad_r = new TH1F("h1_rad_r", "Radial distance of vertices; distance [cm]", 100, 0, 20);
